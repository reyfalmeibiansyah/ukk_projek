                ...
                <div class="row align-items-stretch">
                    ...
                    <div class="col-md-6 mb-4 mb-md-0 d-flex flex-column justify-content-between"
                        ...
                        <div>
                            ...
                            @if (isset($daftarProduk) && count($daftarProduk) > 0)
                                @foreach ($daftarProduk as $produk)
                                    <div class="d-flex justify-content-between border-bottom py-2">
                                        <div>
                                            <p class="mb-0 fw-semibold">{{ $produk['title'] }}</p>
                                            <small class="text-muted">Rp. {{ number_format($produk['price']) }} x
                                                {{ $produk['jumlah'] }}</small>
                                        </div>
                                        <p class="fw-semibold mb-0">Rp. {{ number_format($produk['subtotal']) }}</p>
                                    </div>
                                @endforeach

                                <div class="d-flex justify-content-between mt-3">
                                    <h6 class="fw-semibold">Total</h6>
                                    <h5 class="fw-bold">Rp. {{ number_format($totalKeseluruhan) }}</h5>
                                </div>
                            @else
                    ...
                    <div class="col-md-6">
                        <form id="formPenjualan" method="GET">
                            ...
                            <input type="hidden" name="total_price" value="{{ $totalKeseluruhan }}">
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select" name="is_member" id="memberStatus">
                                    <option value="bukan_member"
                                        {{ old('is_member') === 'bukan_member' ? 'selected' : '' }}>Bukan Member</option>
                                    <option value="member" {{ old('is_member') === 'member' ? 'selected' : '' }}>Member
                                    </option>
                                </select>
                            </div>
                            ...
                            <div class="mb-3 {{ old('is_member') === 'member' ? '' : 'd-none' }}" id="phoneInput">
                                ...
                                <input type="text" name="visible_phone" id="visiblePhone" class="form-control"
                                    placeholder="Masukkan No. HP" value="{{ old('customer_phone') }}">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="number" name="total_payment" class="form-control"
                                    value="{{ old('total_payment', $totalKeseluruhan) }}" required>
                                @error('total_payment')
                                    <small class="text-danger">{{ $message }}</small>
                                @enderror
                            </div>
